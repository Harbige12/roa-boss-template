...
if (timer * img_spd <= sprite_get_number(spark_sprite)) {
    draw_sprite_ext(spark_sprite, timer * img_spd, round(x), round(y), 1, 1, 0, get_player_hud_color(player), 1);
}

img_spd = 0.1;
if (timer * img_spd <= sprite_get_number(lines_sprite)) {
    draw_sprite_ext(lines_sprite, timer * img_spd, view_get_xview() + view_get_wview() / 2, view_get_yview() + view_get_hview() / 2, 1, 1, 0, get_player_hud_color(player), 1);
}